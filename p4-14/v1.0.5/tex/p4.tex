...
%\title{\sffamily\bfseries\huge The P4 Language Specification\\ \vspace{3mm} \Large Version 1.0.5}
...
\SUBSUBSECTION{Parallel and Sequential Semantics}{actionsemantics}
...
\SUBSUBSECTION{Summary of changes introduced in 1.0.5}{change-summary-1.0.5}

\begin{itemize}
\item LPM match (Section \ref{sec:tabledecs})
\begin{itemize}
\item Add a constraint to allow only one \texttt{lpm} match type per table.
\end{itemize}
...
\end{itemize}
\item Primitive action on direct meters (Section \ref{sec:primitiveactions})
\begin{itemize}
\item Remove the sentence that defined semantics for applying \texttt{execute_meter} primitive action on direct meters
and state it is not allowed.
\end{itemize}
\end{itemize}

\SUBSUBSECTION{Summary of changes introduced in 1.0.4}{change-summary-1.0.4}
